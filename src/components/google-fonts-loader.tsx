 ...
 *
 * Performance optimized: Uses media="print" trick to prevent render-blocking,
 * then switches to "all" once loaded. Combined with font-display:swap for optimal performance.
 */
export function GoogleFontsLoader() {
  ...
  useEffect(() => {
    ...
    const existingLink = document.querySelector(
      'link[href*="fonts.googleapis.com/css2"][href*="Inter"]'
    ) as HTMLLinkElement;

    if (existingLink && existingLink.media !== "print") {
      // Fonts already loaded and active, no need to load again
      return;
    }
    ...
    // Performance optimization: Set media to "print" initially to prevent render-blocking
    // This allows the page to render immediately while fonts load in the background
    link.media = "print";

    // Function to activate fonts (switch media to "all")
    const activateFonts = () => {
      if (link.media !== "all") {
        link.media = "all";
      }
    ...
    // Use onload to activate fonts once loaded (most reliable method)
    link.onload = () => {
      activateFonts();
      // Also verify with FontFace API if available
    ...
    // Fallback: Use onerror as fallback (though it shouldn't fire for CSS)
    link.onerror = () => {
      // Even on error, try to activate (might be a false error)
      activateFonts();
    };
    ...
    // Append to head - fonts will start loading (but won't block rendering)
    document.head.appendChild(link);

    // Fallback timeout: If onload doesn't fire (rare edge case), activate after 3 seconds
    // This ensures fonts are activated even if onload event fails
