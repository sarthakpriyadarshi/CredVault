    ...
    try {
      ...
      if (!checkResponse.ok) {
        ...
        if (checkData.error) {
          ...
          if (
            checkData.error.includes("verify") &&
            checkData.error.includes("email")
          ) {
      ...
      if (result?.error) {
        ...
        if (
          ...
          errorMessage =
            "Please verify your email address before signing in. Check your inbox for the verification link. If you don't see it, check your spam folder.";
          setShowResendButton(true);
        } else if (
          errorStr.includes("Invalid role") ||
          errorStr.includes("registered as")
        ) {
          setShowResendButton(false);
          ...
          if (parts) {
            ...
            errorMessage =
              "Invalid role for this login page. Please use the correct login page.";
          }
        } else if (errorStr) {
          ...
          if (
            cleaned &&
            cleaned.length > 0 &&
            cleaned !== "CallbackRouteError"
          ) {
            // If it's a generic error that might be email verification, check it
            if (
              cleaned.includes("Configuration") ||
              cleaned.includes("CallbackRouteError")
            ) {
              errorMessage =
                "Please verify your email address before signing in. Check your inbox for the verification link. If you don't see it, check your spam folder.";
              setShowResendButton(true);
              ...
              >
                ...
                <div className="mt-6 text-center">
                  <p className="text-sm text-zinc-500">
                    Admin access only. Contact system administrator if you need
                    access.
                  </p>
                </div>
