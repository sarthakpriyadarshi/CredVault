  ...
  return (
    ...
    >
      <title id="Vault Protocol">
        {verified ? "Verified on VAULT Protocol" : "Not verified on blockchain"}
      </title>
      <rect
        x="20"
        y="20"
...
export default function ManageCredentialsPage() {
  ...
  const loadCredentials = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/v1/issuer/credentials?${params.toString()}`,
        {
          credentials: "include",
        }
      );
          ...
          credentialsList.map(
            (cred: {
              ...
              issuedAt: cred.issuedAt
                ? new Date(cred.issuedAt).toISOString()
  ...
  const loadMoreCredentials = async () => {
    ...
    try {
      ...
      const newCredentials = credentialsList.map(
        (cred: {
          id?: string;
          _id?: { toString: () => string };
          templateName?: string;
  ...
  const handleRevoke = async () => {
    ...
    try {
      const res = await fetch(
        `/api/v1/issuer/credentials/${selectedCredential.id}/revoke`,
        {
          method: "PUT",
          credentials: "include",
        }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setModalMessage(
          errorData.error ||
            errorData.details ||
            "Failed to verify credential on blockchain"
        );
        setShowErrorModal(true);
        return;
      }
      ...
      setModalMessage(
        "An error occurred while verifying credential on blockchain"
      );
      setShowErrorModal(true);
    } finally {
      setVerifying(false);
              ...
              <div className="flex items-center justify-between">
                <div className="space-y-2 min-w-0">
                  ...
                  <p className="text-muted-foreground">
                    View, manage, and revoke issued credentials
                  </p>
                </div>
              </div>
                          ...
                          <TableCell>
                            {credential.isOnBlockchain ? (
                              <div className="flex items-center gap-2">
                                ...
                                <VaultIcon
                                  verified={credential.blockchainVerified}
                                  size={20}
                                />
                              </div>
                            ) : (
                              ...
                              {credential.isOnBlockchain &&
                                !credential.blockchainVerified && (
                                  <Button
                                    ...
                                    size="sm"
                                    onClick={() =>
                                      handleVerifyBlockchain(credential)
                                    }
                                    disabled={verifying}
                                  ...
                                  onClick={() => {
                                    ...
                                    if (url) {
                                      const filename = credential.certificateUrl
                                        ? `${credential.templateName.replace(
                                            /\s+/g,
                                            "_"
                                          )}_certificate.png`
                                        : `${credential.templateName.replace(
      ...
      >
        <DialogContent className="bg-card border-border/50">
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to revoke this credential? This action
              cannot be undone. The recipient will no longer be able to verify
              or use this credential.
            </DialogDescription>
          </DialogHeader>
          {selectedCredential && (
            <div className="space-y-2 p-4 bg-background/50 rounded-lg">
              <p className="text-sm">
                ...
                {selectedCredential.templateName}
              </p>
              <p className="text-sm">
                <span className="font-semibold">Recipient:</span>{" "}
                {selectedCredential.recipientEmail}
              </p>
              <p className="text-sm">
            ...
            <div className="flex flex-col h-full">
              ...
              <div className="p-6 border-b border-border/50 shrink-0">
                ...
                <DialogDescription className="mt-1">
                  Credential for {selectedCredential.recipientName} (
                  {selectedCredential.recipientEmail})
                </DialogDescription>
              </div>
