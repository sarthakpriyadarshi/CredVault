...
export default function IssuePage() {
      ...
      const fetchTemplateDetails = async () => {
        try {
          const res = await fetch(
            `/api/v1/issuer/templates/${selectedTemplate}`,
            {
              credentials: "include",
            }
          );
          ...
          if (res.ok) {
            ...
            const fields = (template.placeholders || template.fields || []).map(
              (f: { fieldName?: string; name?: string; type?: string }) => ({
  ...
  const handleIssue = async () => {
    ...
    const hasEmail = emailFields.some(
    ...
    const nameField = activeTemplate.fields.find(
      (f) => f.name.toLowerCase().trim() === "name"
    );
    if (
      nameField &&
      activeTemplate.type !== "badge" &&
      (!formData[nameField.name] || !formData[nameField.name].trim())
    ) {
    ...
    if (
      issueDateField &&
      (!formData[issueDateField.name] || !formData[issueDateField.name].trim())
    ) {
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-2">
                ...
                <p className="text-muted-foreground">
                  Fill in the details to issue a single credential
                </p>
              </div>
                                ...
                                <div key={field.name} className="space-y-2">
                                  ...
                                  {isIssueDate && (
                                    <p className="text-xs text-muted-foreground">
                                      Issue Date is automatically set to
                                      today&apos;s date and cannot be modified
                                    </p>
                                  )}
                                </div>
                              );
                          ...
                          <div className="flex-1">
                            ...
                            {!blockchainEnabled && (
                              <p className="text-xs text-muted-foreground mt-1">
                                Contact your admin to enable blockchain for your
                                organization
                              </p>
                            )}
                          </div>
                  ...
                  <Card className="p-6 border border-border/50 bg-card/50 backdrop-blur">
                    ...
                    <ul className="text-sm text-muted-foreground space-y-3">
                      ...
                      <li className="flex gap-2">
                        <span className="text-primary">•</span>
                        <span>
                          Enable blockchain for permanent verification
                        </span>
                      </li>
                      <li className="flex gap-2">
                        <span className="text-primary">•</span>
