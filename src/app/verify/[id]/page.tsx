...
import { CheckCircle, XCircle, Shield, Download, ExternalLink, Share2, Blocks, Loader2 } from "lucide-react"
  ...
  const handleVerifyBlockchain = async () => {
    ...
    try {
      ...
      if (data.verified && data.blockchainVerified) {
        ...
        if (verification) {
          setVerification({
            ...
            details: {
              ...
              blockchainVerifiedAt: data.blockchainVerifiedAt || new Date().toISOString(),
            }
          })
        }
        ...
        setDialogMessage("Blockchain verification is still pending. Please try again later.")
        setDialogOpen(true)
      }
    } catch (error) {
              ...
              <div className="lg:w-[30%]">
                <Card className="p-6 border border-border/50 bg-card/50 backdrop-blur h-full">
                  <div className="space-y-6">
                    ...
                    <div>
                      ...
                      <div
                        className={`flex items-center justify-center gap-2 px-3 py-2 rounded-lg ${
                          verification.verified || (verification.method === "blockchain" && verification.details?.blockchainVerified)
                      ...
                      >
                        ...
                        <span className="font-medium text-sm">
                          {verification.verified || (verification.method === "blockchain" && verification.details?.blockchainVerified) ? "Verified" : "Not Verified"}
                        </span>
                      </div>
                      
                      {/* Verify Button - Always show for blockchain credentials */}
                      {verification.method === "blockchain" && 
                       (verification.details?.vaultFid || verification.details?.transactionId) && (
                        <PrimaryButton 
                    ...
                    {verification.method === "blockchain" && verification.details?.transactionId && (
                        ...
                        <div className="text-center">
                          ...
                          >
                            {verification.details.transactionId.length > 20 
                              ? `${verification.details.transactionId.slice(0, 10)}...${verification.details.transactionId.slice(-10)}`
                      ...
                      <div className="flex flex-col items-center text-center gap-2">
                        ...
                        <div>
                          <p className="text-xs font-medium text-foreground">
                            <span className="capitalize">{verification.method}</span> Verification
                            {verification.method === "blockchain" && verification.details?.blockchainVerified && (
                      ...
                      {verification.method === "blockchain" && verification.details?.transactionId && (
                        <div className="pt-3 mt-3 border-t border-border/50 text-center">
                          <p className="text-sm text-muted-foreground flex items-center justify-center gap-2">
                            <svg width="32" height="32" viewBox="15 15 70 70" xmlns="http://www.w3.org/2000/svg" aria-labelledby="Vault Protocol" role="img" className="shrink-0">
                              <title id="Vault Protocol">Vault Protocol</title>
                              <rect x="20" y="20" width="60" height="60" rx="10" ry="10" fill="oklch(0.32 0 0)"/>
                              <circle cx="50" cy="50" r="22" fill="oklch(0.70 0.20 15)"/>
                              <rect x="36" y="45" width="6" height="11" rx="1.5" ry="1.5" fill="oklch(0.1797 0.0043 308.1928)"/>
                              <rect x="58" y="45" width="6" height="11" rx="1.5" ry="1.5" fill="oklch(0.1797 0.0043 308.1928)"/>
                              <rect x="43" y="49" width="14" height="3" rx="0.5" ry="0.5" fill="oklch(0.1797 0.0043 308.1928)"/>
                            </svg>
                            Powered by VAULT Protocol
                          </p>
                          <a
                  ...
                  <div className="space-y-3">
                      ...
                      <div className="p-3 bg-emerald-500/10 border border-emerald-500/30 rounded-lg">
                        ...
                        {verification.details.blockchainVerifiedAt && (
                          <p className="text-xs text-muted-foreground mt-2">
                            {new Date(verification.details.blockchainVerifiedAt).toLocaleString("en-US", {
                              year: "numeric",
                              month: "short",
                              day: "numeric",
                              hour: "2-digit",
                    ...
                    {verification.details.vaultFid && (
                      <div className="p-2 bg-background/50 rounded-lg">
                        <p className="text-xs text-muted-foreground mb-0.5">VAULT File ID (FID)</p>
                    ...
                    {verification.details.vaultCid && (
                      <div className="p-2 bg-background/50 rounded-lg">
                        <p className="text-xs text-muted-foreground mb-0.5">IPFS Content ID (CID)</p>
                        <p className="font-mono text-xs text-foreground break-all">
                          {verification.details.vaultCid}
                        </p>
                      </div>
                      ...
                      <div className="p-2 bg-background/50 rounded-lg">
                        ...
                        >
                          {verification.details.transactionId.length > 30 
                            ? `${verification.details.transactionId.slice(0, 15)}...${verification.details.transactionId.slice(-15)}`
                            : verification.details.transactionId
                          }
                          <ExternalLink className="h-3 w-3 shrink-0" />
                        </a>
