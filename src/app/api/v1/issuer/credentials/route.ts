...
} from "@/lib/services/vault-protocol.service";
import {
  checkVaultHealth,
  getHealthErrorMessage,
} from "@/lib/services/vault-health.service";
...
async function getHandler(
  ...
  try {
    ...
    const formattedCredentials = (credentials as CredentialDoc[]).map(
      (cred) => {
        ...
        return {
          ...
          recipientName:
            cred.credentialData?.Name || cred.credentialData?.name || "Unknown",
          type: cred.type,
          status: cred.status,
...
async function postHandler(
  ...
  try {
    ...
    const body = await parseBody<{
      ...
      badgeUrl?: string; // Optional: uploaded badge image (base64 data URL)
    }>(req);
    ...
    if (!emailField) {
      return NextResponse.json(
        { error: "Template must have an email field" },
        { status: 400 }
      );
    }
    ...
    const hasQRCodePlaceholder = template.placeholders.some(
      (p) => p.type === "qr" && p.x !== undefined && p.y !== undefined
    );
    ...
    if (hasQRCodePlaceholder) {
      try {
        ...
        credentialIdForQR = new mongoose.Types.ObjectId(
        ...
        const qrPlaceholders = template.placeholders.filter(
          (p) => p.type === "qr"
        );
        ...
        if (
          (template.type === "certificate" || template.type === "both") &&
        ...
        // If badge is uploaded directly, use it; otherwise generate from template
        if (template.type === "badge" || template.type === "both") {
          if (uploadedBadgeUrl) {
            // Use uploaded badge directly
            badgeUrl = uploadedBadgeUrl;
          } else if (template.badgeImage) {
        ...
        if (useBlockchain && certificateUrl) {
          try {
            ...
            const extension =
              VaultProtocolService.getFileExtension(certificateUrl);
            const fileName = `certificate_${recipientEmail}_${Date.now()}${extension}`;

            const vaultResponse = await vaultProtocol.issueCertificate(
              buffer,
              fileName,
              recipientEmail
            );
        ...
        // Upload badge to blockchain if certificate wasn't uploaded
        ...
        tempCredential.blockchainNetwork = useBlockchain
          ? "VAULT Protocol / Quorum"
          : undefined;
        ...
        return NextResponse.json(
          {
            error: `Failed to issue credential with QR code: ${
        ...
        if (
          ...
          if (displayedPlaceholders.length > 0) {
            ...
            if (useBlockchain && certificateUrl) {
              try {
                const buffer =
                  VaultProtocolService.base64ToBuffer(certificateUrl);
                const extension =
                  VaultProtocolService.getFileExtension(certificateUrl);
                const fileName = `certificate_${recipientEmail}_${Date.now()}${extension}`;
                ...
                console.log(
                  "VAULT Protocol Response (Certificate):",
                  JSON.stringify(vaultResponse, null, 2)
                );
                ...
                if (vaultResponse.success) {
                  ...
                  console.log("Assigned VAULT values (Certificate):", {
                    vaultFid,
                    vaultCid,
                    vaultUrl,
                    blockchainTxId,
          ...
          if (useBlockchain && badgeUrl && !vaultFid) {
            try {
              ...
              if (vaultResponse.success) {
                ...
                console.log("Assigned VAULT values (Badge):", {
                  vaultFid,
                  vaultCid,
                  vaultUrl,
                  blockchainTxId,
      ...
      const credential = await Credential.create({
        ...
        blockchainNetwork: useBlockchain
          ? "VAULT Protocol / Quorum"
          : undefined,
        status: "active",
        issuedAt: new Date(),
    ...
    try {
      ...
      const emailHtml = generateCredentialIssuedEmail({
        ...
        }/verify/${credential._id.toString()}`,
      ...
      if (recipientUser) {
        await createNotification({
          ...
          message: `You have been issued a new ${
            template.name || "credential"
          } by ${issuerOrganization}.`,
          link: `/verify/${credential._id.toString()}`,
        });
      }
    } catch (emailError) {
      console.error(
        "Failed to send credential notification email:",
        emailError
      );
